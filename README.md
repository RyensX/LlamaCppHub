...
LlamaCppHub 是一个方便且好看的 `llama.cpp`用户界面外壳，可以便捷地管理`llama.cpp`运行配置。

和LM Studio等软件不同，本软件仅仅只是个外壳，不自带`llama.cpp`运行时，因此可以自由地组合使用各种不同分支的`llama.cpp`，最大程度地提高性能。

> 本软件大部份代码也是由本地部署的`Qwen3.6-35B-A3B`编写，既方便本地推理，又能反过来测试本地模型能力。
